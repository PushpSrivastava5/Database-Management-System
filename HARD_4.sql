...
graph_edges as (
    select table1.subject_id as from_node, table2.subject_id as to_node 
...
,  Reaches(from_node, to_node, path_length) as (
    select
        from_node, to_node, 1 AS path_length
    from
        graph_edges
    where from_node = 18237734
    ...
    where R1.path_length < 3 -- Limit the recursion to 3 iterations
)

select
    case
        when exists (select 1 from (select * from Reaches where to_node = 13401124 and path_length = 3) as t) then 'True'
        else 'False'
    end as pathexists